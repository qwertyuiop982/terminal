 ...
 *
 * HOME is files/home and USR is files/usr. Both live under the application
 * data directory. Executable files are not stored there: Android 10 refuses
 * execve() from an app home directory for targetSdk 29+, so dash is loaded
 * from the native-library directory and later commands should be installed
 * under files/usr/libexec, which keeps the executable bit.
 */
object Rootfs {
    fun ensure(context: Context): Layout {
        ...
        return Layout(home, usr, shell(context))
    }
    ...
    private fun shell(context: Context): File {
        val abi = Build.SUPPORTED_ABIS.firstOrNull() ?: "arm64-v8a"
        val native = File(context.applicationInfo.nativeLibraryDir, "libdash.so")
        ...
        val extracted = File(context.codeCacheDir, "dash")
